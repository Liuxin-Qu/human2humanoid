...
from phc.utils.torch_g1_23_humanoid_batch import Humanoid_G1_23_Batch, G1_23_ROTATION_AXIS

device = torch.device("cpu")
g1_23_joint_names = [ 'pelvis', 
                   ...
                   'torso_link', 'left_shoulder_pitch_link', 'left_shoulder_roll_link', 'left_shoulder_yaw_link', 'left_elbow_link', 'left_wrist_roll_rubber_hand',
...
g1_23_joint_pick_idx = [ g1_23_joint_names_augment.index(j) for j in g1_23_joint_pick]
smpl_joint_pick_idx = [SMPL_BONE_ORDER_NAMES.index(j) for j in smpl_joint_pick]


#### Preparing fitting varialbes

pose_aa_g1_23 = np.repeat(np.repeat(sRot.identity().as_rotvec()[None, None, None, ], 24, axis = 2), 1, axis = 1)
pose_aa_g1_23 = torch.from_numpy(pose_aa_g1_23).float()

dof_pos = torch.zeros((1, 23))
pose_aa_g1_23 = torch.cat([torch.zeros((1, 1, 3)), G1_23_ROTATION_AXIS * dof_pos[..., None], torch.zeros((1, 2, 3))], axis = 1)


root_trans = torch.zeros((1, 1, 3))    

###### prepare SMPL default pause for G1_23
...
fk_return = g1_23_fk.fk_batch(pose_aa_g1_23[None, ], root_trans_offset[None, 0:1])
...
os.makedirs("data/g1_23", exist_ok=True)
joblib.dump((shape_new.detach(), scale), "data/g1_23/shape_optimized_v1.pkl") # V2 has hip jointsrea
print(f"shape fitted and saved to data/g1_23/shape_optimized_v1.pkl")