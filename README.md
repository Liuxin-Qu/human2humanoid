...
3. Different Configurations on **Hisotry Steps**
    ...
    **0-step MLP**
    ...
    env.num_observations=90 \
    env.num_privileged_obs=167 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
    motion.teleop_selected_keypoints_names=[] \
    motion.extend_head=True \
    num_envs=4096 \
    ...
    **5-step MLP**
    ...
    --config-name=config_teleop \
    task=h1:teleop \
    run_name=OmniH2O_STUDENT_50stepMLP \
    env.num_observations=405 \
    env.num_privileged_obs=482 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
    motion.teleop_selected_keypoints_names=[] \
    motion.extend_head=True \
    num_envs=4096 \
    ...
    --config-name=config_teleop \
    task=h1:teleop \
    env.env.env.num_observations=405 \
    env.num_privileged_obs=482 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
    motion.teleop_selected_keypoints_names=[] \
    motion.extend_head=True \
    ...
    **50-step MLP**
    ...
    task=h1:teleop \
    run_name=OmniH2O_STUDENT_50stepMLP \
    env.num_observations=3240 \
    env.num_privileged_obs=3317 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
    motion.teleop_selected_keypoints_names=[] \
    motion.extend_head=True \
    num_envs=4096 \
...
4. Different Configurations on **Hisotry Architectures**
    ...
    **LSTM**
    ...
    train.policy.init_noise_std=0.001 \
    env.add_short_history=False \
    env.short_history_length=0 \
    noise.add_noise=False \
    noise.noise_level=0 \
    train.dagger.load_run_dagger=TEACHER_RUN_NAME \
    train.dagger.checkpoint_dagger=55500 \
    train.dagger.dagger_only=True \
    train.runner.policy_class_name=ActorCriticRecurrent \
    train.policy.rnn_type=lstm
    ```
...
5. Different Configurations on **Observation Type (tracking points)**
    ...
    --config-name=config_teleop \
    task=h1:teleop \
    run_name=OmniH2O_STUDENT_8point \
    env.num_observations=1719 \
    env.num_privileged_obs=1796 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
    ...
    --config-name=config_teleop \
    task=h1:teleop \
    run_name=OmniH2O_STUDENT_23point \
    env.num_observations=1845 \
    env.num_privileged_obs=1922 \
    motion.teleop_obs_version=v-teleop-extend-vr-max-nolinvel \
...
6. Different Configurations on **Observation Type (with linear velocity)**
    ...
    task=h1:teleop \
    run_name=OmniH2O_STUDENT \
    env.num_observations=1743 \
    env.num_privileged_obs=1820 \
    motion.teleop_obs_version=v-teleop-extend-vr-max \
    motion.teleop_selected_keypoints_names=[] \
    motion.extend_head=True \
    num_envs=4096 \
    asset.zero_out_far=False \
