...
class Humanoid_G1_23_Batch:

    def __init__(self, mjcf_file = f"resources/robots/g1_description/g1_23dof.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):
        self.mjcf_data = mjcf_data = self.from_mjcf(mjcf_file)
        self.extend_hand = extend_hand
        self.extend_head = extend_head
        if extend_hand:
            self._parents = mjcf_data['parent_indices'].to(device)
        ...
        if extend_head:
            ...
            self._parents = torch.cat((self._parents, torch.tensor([0]).to(device))).to(device) # Adding the head joints
            head_length = 0.5
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        pose = pose[..., :len(self._parents), :] # G1_23 fitted joints might have extra joints
        if self.extend_hand and self.extend_head and pose.shape[-2] == 24:
            pose = torch.cat([pose, torch.zeros(B, seq_len, 1, 3).to(device).type(dtype)], dim = -2) # adding hand and head joints
        # print("pose.shape[-2]",pose.shape[-2])
        ...
        if return_full:
            ...
            # print(rigidbody_linear_velocity)
